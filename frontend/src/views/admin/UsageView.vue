  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <div class="mb-4 flex gap-2 border-b border-gray-200 dark:border-dark-700">
        <button class="tab" :class="{ 'tab-active': recordsTab === 'usage' }" @click="recordsTab = 'usage'">
          {{ t('usage.tabs.usage') }}
        </button>
        <button class="tab" :class="{ 'tab-active': recordsTab === 'errors' }" @click="switchToErrorsTab">
          {{ t('usage.tabs.errors') }}
        </button>
      </div>
      <div v-show="recordsTab === 'usage'">
        <UsageTable
          :data="usageLogs"
          :loading="loading"
...
const refreshData = () => {
  ...
  loadModelStats(modelDistributionSource.value, true)
  loadChartData()
  if (recordsTab.value === 'errors') loadAdminErrors()
}
const resetFilters = () => {
...
// Error tab state
type UsageRecordsTab = 'usage' | 'errors'
const recordsTab = ref<UsageRecordsTab>('usage')
...
const switchToErrorsTab = () => { recordsTab.value = 'errors'; if (errRows.value.length === 0) loadAdminErrors() }
